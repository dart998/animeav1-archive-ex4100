...
type Server struct { db *database.DB; crawl *crawler.Service; mirror *sitemirror.Mirror; tmpl *template.Template; static string; libraryRoot string; siteRoot string }
...
type adminData struct { State crawler.State; Mirror sitemirror.State; MALUsername string; Library []database.LibraryItem; Watching []database.MALWatchingItem }

func New(db *database.DB,c *crawler.Service,mirror *sitemirror.Mirror,webDir,libraryRoot,siteRoot string)(*Server,error){
	...
	return &Server{db:db,crawl:c,mirror:mirror,tmpl:t,static:filepath.Join(webDir,"static"),libraryRoot:libraryRoot,siteRoot:siteRoot},nil
}
func (s *Server) Handler()http.Handler{
	m:=http.NewServeMux()
	m.Handle("/static/",http.StripPrefix("/static/",http.FileServer(http.Dir(s.static))))
	m.Handle("/site/",http.StripPrefix("/site/",http.FileServer(http.Dir(s.siteRoot))))
	m.HandleFunc("/healthz",s.health);m.HandleFunc("/api/status",s.status)
	m.HandleFunc("/admin/settings",s.settings);m.HandleFunc("/admin/rescan",s.rescan);m.HandleFunc("/admin/sync-mal",s.syncMAL);m.HandleFunc("/admin/mirror",s.startMirror);m.HandleFunc("/admin",s.admin);m.HandleFunc("/",s.index)
	return m
}
...
func (s *Server) status(w http.ResponseWriter,r *http.Request){w.Header().Set("Content-Type","application/json");_=json.NewEncoder(w).Encode(map[string]any{"crawler":s.crawl.State.Snapshot(),"mirror":s.mirror.Snapshot()})}
...
func (s *Server) admin(w http.ResponseWriter,r *http.Request){if r.URL.Path!="/admin"{http.NotFound(w,r);return};items,e:=s.db.Library();if e!=nil{http.Error(w,e.Error(),500);return};watching,e:=s.db.MALWatching();if e!=nil{http.Error(w,e.Error(),500);return};d:=adminData{State:s.crawl.State.Snapshot(),Mirror:s.mirror.Snapshot(),MALUsername:s.db.GetSetting("mal_username"),Library:items,Watching:watching};if e=s.tmpl.ExecuteTemplate(w,"admin.html",d);e!=nil{http.Error(w,e.Error(),500)}}
