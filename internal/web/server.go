...
type adminData struct { State crawler.State; MALUsername string; Library []database.LibraryItem; Watching []database.MALWatchingItem }
...
func (s *Server) Handler()http.Handler{m:=http.NewServeMux();m.Handle("/static/",http.StripPrefix("/static/",http.FileServer(http.Dir(s.static))));m.HandleFunc("/healthz",s.health);m.HandleFunc("/api/status",s.status);m.HandleFunc("/admin/settings",s.settings);m.HandleFunc("/admin/rescan",s.rescan);m.HandleFunc("/admin/sync-mal",s.syncMAL);m.HandleFunc("/admin",s.admin);m.HandleFunc("/",s.index);return m}
...
func (s *Server) admin(w http.ResponseWriter,r *http.Request){if r.URL.Path!="/admin"{http.NotFound(w,r);return};items,e:=s.db.Library();if e!=nil{http.Error(w,e.Error(),500);return};watching,e:=s.db.MALWatching();if e!=nil{http.Error(w,e.Error(),500);return};d:=adminData{State:s.crawl.State.Snapshot(),MALUsername:s.db.GetSetting("mal_username"),Library:items,Watching:watching};if e=s.tmpl.ExecuteTemplate(w,"admin.html",d);e!=nil{http.Error(w,e.Error(),500)}}
...
func (s *Server) syncMAL(w http.ResponseWriter,r *http.Request){if r.Method!=http.MethodPost{http.Error(w,"method not allowed",405);return};if e:=s.crawl.RunAll(context.Background());e!=nil{http.Redirect(w,r,"/admin",http.StatusSeeOther);return};http.Redirect(w,r,"/admin",http.StatusSeeOther)}
