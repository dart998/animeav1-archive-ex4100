...
func New(db *database.DB, c *crawler.Service, mirror *sitemirror.Mirror, webDir, libraryRoot, siteRoot, version, commitSHA string) (*Server, error) {
	...
	s := &Server{db: db, crawl: c, mirror: mirror, av1: animeav1.New("https://animeav1.com"), tmpl: t, static: filepath.Join(webDir, "static"), libraryRoot: libraryRoot, siteRoot: siteRoot, version: version, commitSHA: commitSHA}
...
func (s *Server) status(w http.ResponseWriter, r *http.Request) {
	...
	_ = json.NewEncoder(w).Encode(map[string]any{
		...
		"animeav1_session_configured": s.mirror.HasSessionCookie(), "animeav1_library_updated": s.db.GetSetting("animeav1_library_updated"),
	})
}
