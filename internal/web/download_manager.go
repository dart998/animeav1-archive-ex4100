...
func (s *Server) downloadSeriesAPI(w http.ResponseWriter,r *http.Request){
	...
	if lib,e:=libraryindex.Scan(s.libraryRoot);e==nil{_=s.db.ReplaceLibrary(lib)};force:=r.FormValue("force_unplayable")=="1";if !force{var bad []int;for ep:=1;ep<=total;ep++{if info,e:=s.localEpisode(slug,ep);e==nil&&info.Available&&!isDubEpisodeFileName(info.FileName)&&!info.BrowserPlayable{bad=append(bad,ep)}};if len(bad)>0{w.Header().Set("Content-Type","application/json");w.WriteHeader(http.StatusConflict);_=json.NewEncoder(w).Encode(map[string]any{"requires_confirmation":true,"unplayable_episodes":bad});return}}
