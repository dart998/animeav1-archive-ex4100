...
func (s *Server) reconcileDownloadsAPI(w http.ResponseWriter,r *http.Request){
	...
	for _,entry:=range states{
		...
		for i:=range st.Episodes{
			...
			if found[strings.ToLower(filepath.Base(ep.File))]{valid++;result.EpisodesFound++;continue}
...
func (s *Server) episodeMegaDownloadURL(ctx context.Context,slug string,ep int)(string,error){b,e:=s.fetchEpisodeHTML(ctx,slug,ep);if e!=nil{return "",e};for _,m:=range megaDownloadRE.FindAllSubmatch(b,-1){if len(m)<2{continue};raw:=string(m[1]);u,e:=url.Parse(raw);if e!=nil{continue};host,path:=strings.ToLower(u.Hostname()),strings.ToLower(u.Path);if (strings.Contains(host,"mega.nz")||strings.Contains(host,"mega.co.nz"))&&(strings.Contains(path,"/file/")||strings.HasPrefix(u.Fragment,"!")){return raw,nil}};return "",errors.New("sin enlace de descarga Mega para este episodio")}
