...
type onlinePlayer struct { Server string `json:"server"`; URL string `json:"url"` }
...
func (s *Server) onlinePlayerAPI(w http.ResponseWriter, r *http.Request) {
	...
	slug:=strings.TrimSpace(r.URL.Query().Get("slug")); ep,_:=strconv.Atoi(r.URL.Query().Get("episode"))
	if slug==""||ep<1 { http.Error(w,"bad request",400); return }
	players,err:=s.fetchOnlinePlayers(r.Context(),slug,ep)
	if err!=nil { http.Error(w,err.Error(),502); return }
	if len(players)==0 { http.Error(w,"no se encontro un reproductor online",404); return }
	w.Header().Set("Content-Type","application/json"); _=json.NewEncoder(w).Encode(players[0])
}
...
func (s *Server) fetchOnlinePlayers(parent context.Context, slug string, ep int) ([]onlinePlayer,error) {
	...
	req.Header.Set("Accept","text/html,application/xhtml+xml"); req.Header.Set("Accept-Language","es-ES,es;q=0.9")
	...
	b,err:=io.ReadAll(io.LimitReader(resp.Body,8<<20));if err!=nil{return nil,err}
	pref:=map[string]int{"HLS":0,"UPNShare":1,"Voe":2,"Byse":3,"Mega":4,"MP4Upload":5}; seen:=map[string]bool{};out:=[]onlinePlayer{}
	for _,m:=range embedRE.FindAllStringSubmatch(string(b),-1){if len(m)<3||seen[m[2]]{continue};seen[m[2]]=true;if !strings.HasPrefix(m[2],"https://"){continue};out=append(out,onlinePlayer{Server:m[1],URL:m[2]})}
	sort.SliceStable(out,func(i,j int)bool{return pref[out[i].Server]<pref[out[j].Server]}); return out,nil
}
