...
window.addEventListener('click',function(e){var a=e.target&&e.target.closest?e.target.closest('a[href]'):null;if(a&&bad(a.getAttribute('href')||'')){e.preventDefault();e.stopImmediatePropagation()}},true);
...
func (m *Mirror) refreshEasyList() {
	refresh := func() {
		...
		defer cancel()
		if err := m.blocker.Refresh(ctx); err != nil && m.blocker.Count() == 0 {
			m.recordError(fmt.Errorf("EasyList: %w", err))
		}
	}
	refresh()
	t := time.NewTicker(24*time.Hour)
	defer t.Stop()
	for range t.C { refresh() }
}
