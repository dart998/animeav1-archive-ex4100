...
type State struct {
	...
	AV1Series                                    int `json:"animeav1_series"`
	AV1Crawled                                   int `json:"animeav1_crawled"`
...
func (s *State) Snapshot() State {
	s.mu.RLock()
	defer s.mu.RUnlock()
	return State{Running: s.Running, LastStart: s.LastStart, LastFinish: s.LastFinish, LastStatus: s.LastStatus, LastError: s.LastError, AV1Series: s.AV1Series, AV1Crawled: s.AV1Crawled, MALWatching: s.MALWatching, MALMatched: s.MALMatched}
}
...
func (s *Service) RefreshConfigState() {
	...
	if strings.TrimSpace(s.db.GetSetting("animeav1_library_json")) == "" {
		s.State.LastStatus = "waiting_for_animeav1"
		s.State.LastError = "Configura la sesión y actualiza las listas de AnimeAV1 en /admin"
		return
	}
...
func (s *Service) RunAll(ctx context.Context) error {
	...
	items, err := s.animeAV1Scope()
	if err != nil {
		return s.fail("animeav1_scope_error", err)
	}
	if len(items) == 0 {
		s.State.mu.Lock()
		s.State.AV1Series = 0
		s.State.AV1Crawled = 0
		s.State.LastStatus = "waiting_for_animeav1"
		s.State.LastError = "Actualiza las listas de AnimeAV1 en /admin"
		s.State.mu.Unlock()
		return nil
	}
	...
	crawled := 0
	s.State.mu.Lock()
	s.State.AV1Series = len(items)
	s.State.AV1Crawled = 0
	s.State.mu.Unlock()
	for _, item := range items[cursor:end] {
		if strings.TrimSpace(item.Slug) == "" {
			log.Printf("[AV1] %s: omitida porque no tiene slug", item.Title)
			continue
		}
		if err = s.RunTarget(ctx, item.Slug); err != nil {
			log.Printf("[AV1] %s: %v", item.Slug, err)
			continue
		}
		crawled++
		s.State.mu.Lock()
		s.State.AV1Crawled = crawled
		s.State.mu.Unlock()
	}
	next := end
	if next >= len(items) {
	...
	s.State.mu.Lock()
	s.State.LastStatus = "animeav1_crawled"
	s.State.LastError = ""
	s.State.mu.Unlock()
	log.Printf("[AV1] lote %d-%d de %d: %d series rastreadas", cursor+1, end, len(items), crawled)
	return nil
}
func (s *Service) animeAV1Scope() ([]animeav1.Item, error) {
	raw := strings.TrimSpace(s.db.GetSetting("animeav1_library_json"))
	if raw == "" {
		return nil, nil
	}
	var items []animeav1.Item
	if err := json.Unmarshal([]byte(raw), &items); err != nil {
		return nil, fmt.Errorf("listas AnimeAV1 inválidas: %w", err)
	}
	return animeav1.InScope(items), nil
}
...
func (s *Service) watchedItem(slug string) (animeav1.Item, bool) {
	raw := strings.TrimSpace(s.db.GetSetting("animeav1_library_json"))
	var items []animeav1.Item
...
func (s *Service) fetch(ctx context.Context, u string) (string, error) {
	...
	req.Header.Set("User-Agent", "Mozilla/5.0 AnimeAV1Archive/0.1")
